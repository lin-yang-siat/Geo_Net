...
try:
    ...
    from STDLayer import STDLayer3D_LIF
    from att_module import DAM_Module
...
class Encoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x, x_conv1 = self.down1(x)
        x, x_conv2 = self.down2(x)
...
class Geo_Net(nn.Module):
    """
        Geo_Net model combining encoders, distance-attention, and STDLIF.

        Args:
            input_channels (int): Number of channels in the input volume.
            output_channels (int): Number of channels in the final network output.
            feature_channels (int): Base number of feature channels in the encoder/decoder.
            seq_len (int): Sequence length (number of frames) for spatiotemporal data.
        """
    def __init__(self, input_channels: int, output_channels: int, feature_channels: int, seq_len: int):
        ...
        self.STDLIF = STDLayer3D_LIF(2,5)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
                Forward pass for Geo_Net.

                Args:
                    x (Tensor): Input tensor with shape (B, channels, S, D, H, W), where:
                        - B: Batch size
                        - channels: Number of input channels (e.g., 11 for I + P + S)
                        - S: Sequence length (time dimension)
                        - D, H, W: Depth, height, width of the 3D volume

                Returns:
                    Tensor: Output tensor after spiking temporal layer, shape (B, output_channels, D, H, W).
                """
        I = x[:,0:1,:,:,:,:]
        d_P = x[:, 1:2, :, :, :, :]
        d_S = x[:, 2:11, :, :, :, :]
