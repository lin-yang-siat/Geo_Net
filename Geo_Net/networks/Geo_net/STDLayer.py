...
class STDLayer3D(nn.Module):
    ...
    def __init__(
        ...
        """
        super(STDLayer3D, self).__init__()
        ...
        self.sigma2 = nn.Parameter(torch.FloatTensor([10.0] * nb_classes).view(nb_classes, 1, 1,1))
        self.eta = nn.Parameter(torch.FloatTensor([1.0]))
        self.lam = nn.Parameter(torch.FloatTensor([5.0]))

        # Fixed parmaters.
        # self.sigma2 = Variable(torch.FloatTensor([10.0]*nb_classes).view(nb_classes,1,1),requires_grad=False).cuda()
        # self.eta=Variable(torch.FloatTensor([1.0]),requires_grad=False).cuda()
        # self.lam=Variable(torch.FloatTensor([5.0]),requires_grad=False).cuda()
        # self.sigma2 = nn.Parameter(torch.FloatTensor([0.5] * nb_classes).view(nb_classes, 1, 1, 1),
        #                              requires_grad=False)
        # self.eta = nn.Parameter(torch.FloatTensor([1]), requires_grad=False)
        # self.lam = nn.Parameter(torch.FloatTensor([5]), requires_grad=False)
    ...
    def forward(self, o):
        ...
        # std kernel
        ker = STDLayer3D.STD_Kernel(self.sigma2, self.ker_halfsize)

        for i in range(self.nb_iterations):
            # 1. subgradient
            q = F.conv3d(1.0 - 2.0 * u, ker, padding=(0,self.ker_halfsize,self.ker_halfsize), groups=self.nb_classes)
            # 2. Softmax
            u = self.softmax((o - self.lam * q) / (self.eta))

        return u

    def STD_Kernel(sigma, halfsize):
        x, y, z = torch.meshgrid(torch.arange(-halfsize, halfsize + 1), torch.arange(-halfsize, halfsize + 1),
...
if __name__ == "__main__":
    ...
    model = STDLayer3D_LIF(nb_classes=2, nb_iterations=10).cuda()
    input = torch.rand(8, 2, 16, 160, 160).cuda()
    input2 = torch.rand(8, 1, 16, 160, 160).cuda()
